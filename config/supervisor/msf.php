...
stdout_logfile=/vue-msf/data/msf-___NAME___/server.log
redirect_stdout=true
stderr_logfile=/vue-msf/data/msf-___NAME___/error.log
eot;
...
if (!empty($msfServers)) {
    ...
    foreach ($msfServers as $name => $bin) {
        // 创建日志目录
        if (!is_dir('/vue-msf/data/msf-' . $name)) {
            mkdir('/vue-msf/data/msf-' . $name, 0777, true);
        }
    ...
    file_put_contents('/vue-msf/supervisor/conf.d/msf.conf', $newConfContent);
}